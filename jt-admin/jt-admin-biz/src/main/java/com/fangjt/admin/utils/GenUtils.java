package com.fangjt.admin.utils;
...
import com.fangjt.admin.entity.codeGen.GenCategory;
...
import com.fangjt.common.utils.FileUtils;
import com.fangjt.common.utils.FreeMarkerUtils;
import com.fangjt.common.utils.StringUtils;
...
public class GenUtils {
	 ...
	 * @param genTable
	 */
	public static void initColumnField(Model genTable){
		for (ModelField column : genTable.getColumnList()){
...
//			}
			...
			if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
					...
					|| StringUtils.containsIgnoreCase(column.getJdbcType(), "TEXT")//add fang 20160919
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(), "NARCHAR")){
				column.setJavaType("String");
				column.setHtmlShowType("input");
				...
				column.setHtmlShowType("dateselect");
			}else if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "BIGINT")
					||StringUtils.containsIgnoreCase(column.getJdbcType(), "INT")//add fang 0919
					||StringUtils.containsIgnoreCase(column.getJdbcType(), "DECIMAL")//add fang 0919
			...
			if (!StringUtils.equalsIgnoreCase(column.getName(), "id")
					...
					&& !StringUtils.equalsIgnoreCase(column.getName(), "create_user_id")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "update_user_id")
...
//			}
			...
			// 创建时间、更新时间
					...
					|| StringUtils.containsIgnoreCase(column.getName(), "time")
					||  StringUtils.containsIgnoreCase(column.getName(), "date")){
				column.setHtmlShowType("dateselect");
			}
			// 备注、内容
			else if (StringUtils.equalsIgnoreCase(column.getName(), "remarks")
					|| StringUtils.equalsIgnoreCase(column.getName(), "content")){
				column.setHtmlShowType("textarea");
			}
			// 父级ID
			...
			// 删除标记
					...
					|| StringUtils.containsIgnoreCase(column.getName(), "is") ){
				column.setHtmlShowType("radiobox");
	...
	public static <T> T fileToObject(String fileName, Class<?> clazz){
		try {
			String pathName = "/template/gen/" + fileName;
...
//		}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		model.put("className", StringUtils.uncapitalize(genScheme.getModel().getClassName()));
		model.put("ClassName", StringUtils.capitalize(genScheme.getModel().getClassName()));
		
		model.put("functionName", genScheme.getFunctionName());
		model.put("functionNameSimple", genScheme.getFunctionNameSimple());
		model.put("functionAuthor", StringUtils.isNotBlank(genScheme.getFunctionAuthor())?genScheme.getFunctionAuthor():"vnpla");
		model.put("functionVersion", "V0.0.0.1");
		model.put("subModuleName", "");
		
		model.put("urlPrefix", model.get("moduleName")+(StringUtils.isNotBlank("")
				?"/"+StringUtils.lowerCase(""):"")+"/"+model.get("className"));
		model.put("viewPrefix", //StringUtils.substringAfterLast(model.get("packageName"),".")+"/"+
				model.get("urlPrefix"));
		model.put("permissionPrefix", model.get("moduleName")+(StringUtils.isNotBlank("")
				?":"+StringUtils.lowerCase(""):"")+":"+model.get("className"));
		
//		model.put("dbType", CommonConstants.getConfig("jdbc.type"));
		model.put("dbType", "mysql");

		model.put("table", genScheme.getModel());
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println((config.getCategoryList().get(0)).getTemplate());
